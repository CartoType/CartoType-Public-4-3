...
static const std::pair<const char*,uint32_t> VehicleType[] =
    ...
    { "tourist bus", CartoType::KArcBusAccessFlag | CartoType::KArcTouristBusAccessFlag | CartoType::KArcWrongWayFlag },
    { "emergency vehicle", CartoType::KArcEmergencyAccessFlag | CartoType::KArcWrongWayFlag },
    { "vehicle with hazardous material", CartoType::KArcHazardousAccessFlag | CartoType::KArcWrongWayFlag }
    };
...
void RouteProfileDialog::Set(const CartoType::TRouteProfile& aRouteProfile)
    ...
    uint32_t v = aRouteProfile.iVehicleType;
    int index = 0;
    if (v == 0)
        index = 0;
        ...
        index = 8;
    else if (v & CartoType::KArcTouristBusAccessFlag)
        index = 9;
    else if (v & CartoType::KArcEmergencyAccessFlag)
        index = 10;
    else if (v & CartoType::KArcHazardousAccessFlag)
        index = 11;
...
const CartoType::TRouteProfile& RouteProfileDialog::RouteProfile()
    {
    m_route_profile.iVehicleType = VehicleType[m_ui->vehicleTypeList->currentIndex()].second;
