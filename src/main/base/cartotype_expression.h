...
class CExpressionOp
    ...
    /**
    For ENumberOp, the number;
    for EVariableOp, a variable index if >= 0;
    otherwise ignored.
    */
    double iNumber;
...
class CVariableDictionary: public MVariableDictionary
    ...
    void Set(const CString& aVariableName,const CString& aValue)
        {
        if (aValue.Length())
            iDictionary.Insert(aVariableName,aValue);
        else
            iDictionary.Delete(aVariableName);
        }
    template<typename Functor> void Apply(Functor aFunctor) { iDictionary.Apply(aFunctor); }
