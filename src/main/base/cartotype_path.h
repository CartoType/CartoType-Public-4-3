...
class MPath
    ...
    */
    ...
    std::unique_ptr<COutline> Copy() const;
	std::unique_ptr<COutline> ClippedPath(const TRect& aClip) const;
    bool IsSmoothingNeeded() const;
    std::unique_ptr<COutline> SmoothPath() const;
	std::unique_ptr<COutline> FlatPath(double aMaxDistance) const;
    std::unique_ptr<COutline> TruncatedPath(double aStart,double aEnd) const;
    ...
    enum TClipType
        {
        EInside,
        EMayIntersectAndHasNoCurves,
        EMayIntersectAndHasCurves
        };
...
class TContour: public MPath
    ...
    */
    template<typename MTraverser> void Traverse(MTraverser& aTraverser,const TRect* aClip = nullptr) const
        ...
        assert(iPoint->iType != ECubicControlPoint);
	    ...
	    if (point->iType == EQuadraticControlPoint)
		    {
		    /* First point is conic control. Yes, this happens. */
		    if (iPoint[last].iType == EOnCurvePoint)
			    {
	    ...
	    while (point < limit)
		    ...
		    switch (point->iType)
			    ...
			    default: // cubic
				    {
				    if (point + 1 > limit || point->iType != ECubicControlPoint)
					    return; // invalid outline
...
class CContour: public MPath, public MWritableContour
	...
	void AppendPoint(const TOutlinePoint& aPoint)
		{
		if (iPoint.size() && aPoint.iType == EOnCurvePoint && aPoint == iPoint.back())
			return;
	...
	*/
	...
	void AppendCircularArc(const TPoint& aCenter,const TPoint& aStart,
		const TPoint& aEnd,TEllipseAngleType aAngleType = EShortest,bool aAppendStart = false);
	...
	void AppendEllipticalArc(const TPoint& aCenter,TFixed aRX,
		...
		TEllipseAngleType aAngleType = EShortest,bool aAppendStart = false);
...
class CGeometry
    ...
    void Clear() { iPoint.clear(); iContourStart.clear(); iCoordType = EMapCoordType; iClosed = false; }

    TCoordType GetCoordType() const { return iCoordType; }
    size_t ContourCount() const;
    size_t PointCount(size_t aContourIndex) const;
    void GetPoint(size_t aContourIndex,size_t aPointIndex,double& aX,double& aY) const;
    ...
    void BeginContour();
    void GetOutline(COutline& aOutline) const;
    void GetCoordSet(TWritableCoordSet& aCoordSet);
    void GetCoordSet(TCoordSet& aCoordSet) const;
    void Reverse();

    private:
    std::vector<double> iPoint;
    std::vector<size_t> iContourStart;
    TCoordType iCoordType = EMapCoordType;
    bool iClosed = false;
    };
...
class CContourFP
    ...
    void AppendPoint(TPointFP aPoint) { iPoint.push_back(aPoint); }
